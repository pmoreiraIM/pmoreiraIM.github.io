    ...
    <div class="progress" style="height: 30px">
        <div class="progress-bar bg-{{ color_code(epic.forecast.deadline_quantile if epic.forecast else None) }}"
            ...
            aria-valuemax="{{ epic.max_stories }}">
                {{ epic.stories_done }}
                of
                {{ epic.stories_done + epic.stories_in_progress + epic.stories_in_backlog}}
                &ndash;
                stories completed
        </div>
    </div>
...
{%- macro epic_forecast_dates(epic) %}
    ...
    {% endif %}
{%- endmacro %}
{% macro team_progress(team) -%}
    <div class="progress" style="height: 30px">
        <div class="progress-bar bg-{{ team_color_code(team,epics_by_team) }}"
            role="progressbar"
            style="width: {{ team_percent_complete(team,epics_by_team) }}%"
            aria-valuenow="{{ team_stories_done(team,epics_by_team) }}"
            aria-valuemin="0"
            aria-valuemax="{{ team_max_stories(team,epics_by_team) }}">
                {{ team_stories_done(team,epics_by_team) }}
                of
                {{ team_min_stories(team,epics_by_team) }} 
                stories completed
        </div>
    </div>
{%- endmacro %}
{% macro outcome_progress(outcome) -%}
    {% if outcome.epics != [] %}
        <div class="progress" style="height: 30px">
            <div class="progress-bar bg-{{ outcome_color_code(outcome) }}"
                role="progressbar"
                style="width: {{ outcome_percent_complete(outcome) }}%"
                aria-valuenow="{{ outcome_stories_done(outcome) }}"
                aria-valuemin="0"
                aria-valuemax="{{ outcome_max_stories(outcome) }}">
                    {{ outcome_stories_done(outcome) }}
                    of
                    {{ outcome_min_stories(outcome) }} 
                    stories completed
            </div>
        </div>
    {% endif %}
...
<html lang="en">
    ...
    <body>
        ...
        <div class="container mt-4">
            ...
            <h1>{{title}}</h1>
            <p>Generated on: <span id="datetime">{{time_now}}</span></p>
            ...
            <div class="tab-content">
                <div class="tab-pane fade show active" id="by-outcome" role="tabpanel" aria-labelledby="outcome-tab">
                    ...
                    {% for idx, outcome in enumerate(outcomes) %}
                        ...
                        <h4>
                            {{ outcome.epics|select('unresolved_epics')|list|length}} <b>Epics</b> unresolved
                        </h4>
                        {{ outcome_progress(outcome)}}
                        ...
                        <table class="table table-sm">
                            ...
                            <tbody>
                                {% for epic in outcome.epics | sort(attribute='team.name') %}
                                    {% if epic.stories_done != epic.stories_done + epic.stories_in_progress + epic.stories_in_backlog %}
                                        <tr>
                ...
                <div class="tab-pane fade" id="by-team" role="tabpanel" aria-labelledby="team-tab">
                    ...
                    <h2>
                        {{ team.name }} <h4>{{ epics_by_team[team.name]|select('unresolved_epics')|list|length}} <b>Epics</b> unresolved</h4> {{ team_progress(team)}}
                    </h2>
