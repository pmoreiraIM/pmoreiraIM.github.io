...
class ProgressReportCalculator(Calculator):
    ...
    def write(self):
        ...
        jinja_env.tests['unresolved_epics'] = unresolved_epics
        ...
        with open(output_file, 'w') as of:
            of.write(template.render(
                ...
                team_color_code=lambda team,epics_by_team: (
                    'danger' if min([e.forecast.deadline_quantile if e.forecast else 0 for e in epics_by_team[team.name]]) <= 0.7 else
                    'warning' if min([e.forecast.deadline_quantile if e.forecast else 0 for e in epics_by_team[team.name]]) <= 0.9 else
                    'success'
                ),
                outcome_color_code=lambda outcome: (
                    'danger' if min([e.forecast.deadline_quantile if e.forecast else 0 for e in outcome.epics]) <= 0.7 else
                    'warning' if min([e.forecast.deadline_quantile if e.forecast else 0 for e in outcome.epics]) <= 0.9 else
                    'success'
                ),
                percent_complete=lambda epic: (
                    int(round(((epic.stories_done or 0) / epic.max_stories) * 100))
                ),
                team_percent_complete=lambda team,epics_by_team: (
                    int(round(((sum([e.stories_done for e in epics_by_team[team.name]]) or 0) / sum([e.max_stories for e in epics_by_team[team.name]])) * 100))
                ),
                outcome_percent_complete=lambda outcome: (
                    int(round(((sum([e.stories_done for e in outcome.epics]) or 0) / sum([e.max_stories for e in outcome.epics])) * 100))
                ),
                team_stories_done=lambda team,epics_by_team: (
                    sum([e.stories_done for e in epics_by_team[team.name]])
                ),
                outcome_stories_done=lambda outcome: (
                    sum([e.stories_done for e in outcome.epics])
                ),
                team_max_stories=lambda team,epics_by_team: (
                    sum([e.max_stories for e in epics_by_team[team.name]])
                ),
                outcome_max_stories=lambda outcome: (
                    sum([e.max_stories for e in outcome.epics])
                ),
                team_min_stories=lambda team,epics_by_team: (
                    sum([e.min_stories for e in epics_by_team[team.name]])
                ),
                outcome_min_stories=lambda outcome: (
                    sum([e.min_stories for e in outcome.epics])
                ),
                unresolved_epics = lambda epic: (
                    epic.stories_done != epic.stories_done + epic.stories_in_progress + epic.stories_in_backlog 
                ),    
...
def unresolved_epics(epic):
    return epic.stories_done != epic.stories_done + epic.stories_in_progress + epic.stories_in_backlog     